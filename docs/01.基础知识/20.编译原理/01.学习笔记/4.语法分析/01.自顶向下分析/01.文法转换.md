...
- [文法转换](#文法转换)
  - [左递归文法](#左递归文法)
    - [问题： 左递归文法可能会使递归下降分析器陷入无限循环](#问题-左递归文法可能会使递归下降分析器陷入无限循环)
  - [消除直接左递归](#消除直接左递归)
    - [消除直接左递归的一般形式](#消除直接左递归的一般形式)
  - [消除间接左递归](#消除间接左递归)
  - [消除左递归的算法](#消除左递归的算法)
  - [提取左公因子](#提取左公因子)
    - [提取左公因子的算法](#提取左公因子的算法)
...
* 消除左递归是要付出代价的——引进了一些**非终结符**和$\varepsilon-$产生式
...
<h4 id="回溯"> 同一非终结符的多个候选式存在共同前缀，将导致<b>回溯现象</b> </h4>
