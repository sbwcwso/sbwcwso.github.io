...
* 在内存允许的情况下，所能找到的最大质数取决于 `int` 所能表示的最大整数，`int` 一般为 `2**31 - 1 = 2147483647`, 不过由于内存的限制，一般无法达到这个数值。
:::
...
::: details 答案
* 间隔为 `1000` 时结果不明显，将间隔改为 `1000 000`, 计算到 `10 000 000` 为止
* 代码
